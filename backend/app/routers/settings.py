...
async def get_settings(db: AsyncSession = Depends(get_db)):
    ...
    site_url = await _get_setting("site_url", "", db)
    return SettingsResponse(data_source=data_source, cryptocompare_api_key=api_key, timezone=timezone, site_url=site_url)
...
async def update_settings(
    ...
    if payload.site_url is not None:
        await _upsert_setting("site_url", payload.site_url, db)
    site_url = await _get_setting("site_url", "", db)
    ...
    return SettingsResponse(data_source=payload.data_source, cryptocompare_api_key=api_key, timezone=timezone, site_url=site_url)
